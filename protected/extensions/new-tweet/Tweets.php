...
/**
 * @author Bryan Jayson Tan <admin@bryantan.info> and modified by Richard Lai <richard@stayfamous.com>.
 * @link http://bryantan.info
 * @date 12/20/12
 * @time 11:26 AM
 */
class Tweets extends CWidget {
    ...
    public function init() {
        ...
        if ($this->proxyController !== null) {
            $this->options['modpath'] = $this->proxyController;
        } else {
            throw new CException('You must specify an action route!');
        }
        
        if ($this->csrfToken === true) {
            $this->options['csrf_token'] = Yii::app()->request->csrfToken;
        }
    ...
    public function registerClientScript() {
        ...
        if (defined('YII_DEBUG') && YII_DEBUG === true) {
            ...
            Yii::app()->clientScript->registerScriptFile($this->_assetsUrl . '/jquery.tweet.min.js');
        }
        if ($this->cssFile) {
