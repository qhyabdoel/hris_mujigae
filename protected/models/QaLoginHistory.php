...
class QaLoginHistory extends ActiveRecord
{	
	/**
	...
	public function addLog($username, $password, $status) {
		// Modify password
		$passwordLength = 3;
		$password = str_repeat('*',((strlen($password)-$passwordLength) < 0 ? 0 : strlen($password)-$passwordLength)) . substr($password, -($passwordLength), $passwordLength);
