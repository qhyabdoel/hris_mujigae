 ...
 * @return string
 */
function createUrl($url, $params=array()) {
 ...
 * @return string
 */
function createAbsoluteUrl($url, $params=array()) {
...
function registerCoreScript($filename='')
{
	Yii::app()->clientScript->registerCoreScript($filename);
}
...
function yiiexplode($string, $search, $key='value')
{
...
function imageUrl($url='') {
	return getBaseUrl().'/images/'.$url;
}

function imagePaths($path='') {
	return getBasePath().'/images/'.$path;
}

function profileUrl($url='') {
	return imageUrl().'profiles/'.$url;
}
...
function profilePaths($path='') {
	return imagePaths().'profiles/'.$path;
}

function letterPaths($path=''){
	return imagePaths().'letters/'.$path;
}

function cssUrl($url) {
	return getBaseUrl().'/css/'.$url;
}
